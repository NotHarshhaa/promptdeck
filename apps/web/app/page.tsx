    ...
    <main className="min-h-svh bg-background text-foreground">
      <header className="flex h-14 items-center justify-between border-b px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onPress={startNewRun} size="sm"><Plus /> New run <Command className="ml-1 size-3" /></Button>
        </div>
      </header>
      ...
      <div className="grid min-h-[calc(100svh-3.5rem)] lg:grid-cols-[236px_minmax(0,1fr)_320px]">
        <aside className="hidden border-r p-3 lg:flex lg:flex-col">
          ...
          <div className="mt-3">
            <Input value={conversationSearch} onChange={(event) => setConversationSearch(event.target.value)} placeholder="Filter history" aria-label="Filter run history" />
          </div>
          <div className="mt-3 min-h-0 flex-1 space-y-1 overflow-y-auto">
            {visibleConversations.length ? visibleConversations.map((conversation) => (
              ...
              >
                ...
                <p className="mt-1 pl-5 text-[10px] text-muted-foreground">{new Date(conversation.updated_at).toLocaleDateString()}</p>
              </button>
        ...
        <section className="flex min-w-0 flex-col border-r">
          <header className="flex min-h-14 items-center justify-between gap-3 border-b px-4 sm:px-6">
            <div className="flex min-w-0 items-center gap-2">
              <Button variant="outline" size="icon-sm" className="lg:hidden" aria-label="Open navigation"><PanelLeft /></Button>
              <div className="min-w-0">
                <div className="flex items-center gap-2"><span className="size-1.5 bg-foreground" /><p className="truncate text-sm font-medium">{conversationName}</p></div>
                <p className="mt-0.5 text-[10px] tracking-[0.12em] text-muted-foreground uppercase">{conversationId ? `Run ${conversationId.slice(0, 8)}` : "Draft run"}</p>
              </div>
            </div>
            <Button onPress={startNewRun} variant="outline" size="sm"><RotateCcw /> Reset</Button>
          </header>
            ...
            <form onSubmit={submitPrompt} className="border-t bg-background p-4 sm:p-6">
              <div className="mx-auto max-w-3xl border bg-card p-2">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <TooltipTrigger><Button type="button" variant="ghost" size="icon-xs" aria-label="Prompt variables"><Variable /></Button><Tooltip>Variables are available in system instructions.</Tooltip></TooltipTrigger>
                ...
                <div className="space-y-5">
                  <section>
                    ...
                    <div className="grid grid-cols-2 border p-1">
                      <Button type="button" onPress={() => setRunMode("demo")} variant={mode === "demo" ? "default" : "ghost"} size="sm">Demo</Button>
                      <Button type="button" onPress={() => setRunMode("live")} variant={mode === "live" ? "default" : "ghost"} size="sm">Live</Button>
                    </div>
                  ...
                  <section>
                    ...
                    <div className="mt-2 flex h-8 items-center gap-2 border px-2.5"><Braces className="size-3.5 text-muted-foreground" /><span className="truncate font-mono text-[11px]">{mode === "live" ? provider?.live_model : provider?.model ?? "Waiting for provider"}</span></div>
                  </section>
                  <section>
                    <div className="flex items-center justify-between"><FieldLabel>Temperature</FieldLabel><span className="font-mono text-xs">{temperature.toFixed(1)}</span></div>
                    <Slider aria-label="Temperature" minValue={0} maxValue={2} step={0.1} value={temperature} onChange={(value) => setTemperature(Number(value))} className="mt-3" />
                    <div className="mt-1 flex justify-between text-[10px] text-muted-foreground"><span>Precise</span><span>Expansive</span></div>
                  </section>
                  <section>
                    <div className="flex items-center justify-between"><FieldLabel>Maximum tokens</FieldLabel><span className="font-mono text-xs">{maxTokens}</span></div>
                    <Slider aria-label="Maximum output tokens" minValue={256} maxValue={4096} step={256} value={maxTokens} onChange={(value) => setMaxTokens(Number(value))} className="mt-3" />
                    <div className="mt-1 flex justify-between text-[10px] text-muted-foreground"><span>256</span><span>4096</span></div>
                  </section>
                  <section>
                    <FieldLabel>System instruction</FieldLabel>
                    <Textarea value={systemPrompt} onChange={(event) => setSystemPrompt(event.target.value)} className="mt-2 h-24 resize-none" />
                  </section>
                  <Card size="sm" className="border">
                    <CardHeader className="border-b"><CardTitle className="flex items-center gap-2"><Code2 className="size-3.5" />Request contract</CardTitle></CardHeader>
                    <CardContent className="font-mono text-[10px] leading-5 text-muted-foreground">POST /api/chat/stream<br />mode={mode} · temp={temperature.toFixed(1)} · max_tokens={maxTokens}</CardContent>
                  </Card>
                </div>
              </TabsContent>
              <TabsContent id="telemetry" className="pt-4">
                <Card className="border">
                  <CardHeader className="border-b"><CardTitle>Latest run</CardTitle><CardDescription>Values reported after the stream finishes.</CardDescription></CardHeader>
                  <CardContent className="grid grid-cols-3 divide-x pt-4 text-center">
                    <div><p className="font-mono text-sm">{lastRun ? formatTokens(lastRun.prompt_tokens + lastRun.completion_tokens) : "—"}</p><p className="mt-1 text-[10px] text-muted-foreground uppercase">Tokens</p></div>
                    <div><p className="font-mono text-sm">{lastRun ? formatLatency(lastRun.latency_ms) : "—"}</p><p className="mt-1 text-[10px] text-muted-foreground uppercase">Latency</p></div>
                    <div><p className="font-mono text-sm">{lastRun ? formatCost(lastRun.cost_usd) : "—"}</p><p className="mt-1 text-[10px] text-muted-foreground uppercase">Cost</p></div>
                  </CardContent>
                </Card>
