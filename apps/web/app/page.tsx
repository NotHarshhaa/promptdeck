...
function SidebarContent({
  ...
  return (
    <>
      <Button onPress={onStartNewRun} size="sm" className="mb-4 w-full justify-start">
        <Plus /> New run <Command className="ml-auto size-3" />
      </Button>
      ...
      <div className="mt-3 min-h-0 flex-1 space-y-1 overflow-y-auto">
        {visibleConversations.length ? (
          visibleConversations.map((conversation) => (
            <div
              key={conversation.id}
              className={`group flex items-center justify-between border px-2.5 py-1.5 transition-colors ${
            ...
            >
              ...
              <Button
                ...
                onPress={(e) => onDeleteConversation(conversation.id, e as unknown as React.MouseEvent)}
                aria-label={`Delete run ${conversation.title}`}
              >
          ...
          <p className="border border-dashed p-3 text-xs leading-5 text-muted-foreground">
            {conversationSearch ? "No matching runs." : "Completed runs are saved here."}
          </p>
        )}
      </div>
      <Card size="sm" className="mt-4 border">
        <CardContent className="flex items-start gap-2 text-[11px] text-muted-foreground">
          <TerminalSquare className="mt-0.5 size-3.5 shrink-0" />
          Conversations and metrics are stored by the local API.
        </CardContent>
      </Card>
    </>
  )
...
function InspectorPanel({
  ...
  lastRun: RunMetric | null
  feedbackRating: -1 | 1 | null
  submitRunFeedback: (rating: -1 | 1) => Promise<void>
  inspectorTab: string
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-14 items-center gap-2 border-b px-4">
        ...
        <div>
          <p className="text-xs font-medium">Request inspector</p>
          <p className="text-[10px] text-muted-foreground">Configure and review</p>
        </div>
      </div>
            ...
            <div className="space-y-5">
              <section>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground">Server controlled</span>
                </div>
                <div className="grid grid-cols-2 border p-1">
                  <Button
                    type="button"
                    onPress={() => setRunMode("demo")}
                ...
                <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
                  Demo returns a deterministic response. Live is enabled only for configured providers.
                </p>
              </section>
              <section>
                ...
                <select
                  ...
                  className="mt-2 h-8 w-full border bg-background px-2.5 text-xs outline-none focus:border-ring focus:ring-1 focus:ring-ring/50"
                >
                  {providers.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.name} · {item.configured ? "live ready" : "demo only"}
                    </option>
                  ))}
                </select>
                <p className="mt-2 text-[11px] text-muted-foreground">
                  {provider?.configured
                    ? "Live credentials are available on the server."
                    : provider
                      ? `Requires ${provider.required_env} for live mode.`
                      : "Loading provider routes."}
                </p>
              </section>
              <section>
                <FieldLabel>Resolved model</FieldLabel>
                <div className="mt-2 flex h-8 items-center gap-2 border px-2.5">
                  <Braces className="size-3.5 text-muted-foreground" />
                  <span className="truncate font-mono text-[11px]">
                    {mode === "live" ? provider?.live_model : provider?.model ?? "Waiting for provider"}
                  </span>
                </div>
              </section>
              <section>
              ...
              <section>
                <div className="flex items-center justify-between">
                  <FieldLabel>Maximum tokens</FieldLabel>
              ...
              <section>
                <FieldLabel>System instruction</FieldLabel>
                <Textarea
                  value={systemPrompt}
              ...
              <Card size="sm" className="border">
                ...
                <CardContent className="font-mono text-[10px] leading-5 text-muted-foreground">
                  POST /api/chat/stream
                  <br />
                  mode={mode} · temp={temperature.toFixed(1)} · max_tokens={maxTokens}
                </CardContent>
              </Card>
            </div>
          </TabsContent>
          <TabsContent id="telemetry" className="pt-4">
            <Card className="border">
              <CardHeader className="border-b">
                ...
                <CardDescription>Values reported after the stream finishes.</CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-3 divide-x pt-4 text-center">
                <div>
                  <p className="font-mono text-sm">
                    {lastRun ? formatTokens(lastRun.prompt_tokens + lastRun.completion_tokens) : "—"}
                  </p>
                  <p className="mt-1 text-[10px] text-muted-foreground uppercase">Tokens</p>
                </div>
                <div>
                  <p className="font-mono text-sm">{lastRun ? formatLatency(lastRun.latency_ms) : "—"}</p>
                  <p className="mt-1 text-[10px] text-muted-foreground uppercase">Latency</p>
                </div>
                <div>
                  <p className="font-mono text-sm">{lastRun ? formatCost(lastRun.cost_usd) : "—"}</p>
                  <p className="mt-1 text-[10px] text-muted-foreground uppercase">Cost</p>
                </div>
              </CardContent>
            </Card>
            <Card className="mt-4 border" size="sm">
              <CardContent className="text-[11px] leading-5 text-muted-foreground">
                {lastRun
                  ? `${lastRun.token_source} token counts · ${lastRun.mode} mode`
                  : "Run a prompt to populate observed telemetry."}
              </CardContent>
            </Card>
            {lastRun && (
              <Card className="mt-4 border" size="sm">
                <CardContent>
                  <p className="text-xs font-medium">Was this response helpful?</p>
                  <div className="mt-2 flex gap-2">
                    <Button
                      onPress={() => void submitRunFeedback(1)}
...
export default function Page() {
  ...
  async function deleteConversation(id: string, event?: React.MouseEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Could not delete run.")
      setConversations((current) => current.filter((item) => item.id !== id))
      if (conversationId === id) {
        startNewRun()
      }
      setNotice("Run deleted from local history.")
  ...
  function setRunMode(nextMode: RunMode) {
    if (nextMode === "live" && !provider?.configured) {
      setNotice(
        `${provider?.name ?? "This provider"} is not configured. Set ${
          provider?.required_env ?? "its required environment value"
        } on the API, then restart it.`
      )
      return
    }
    setNotice(null)
  ...
  function extractVariablesFromPrompt() {
    ...
    if (detected.size === 0) {
      setNotice("No variable tags found in prompt or system instruction. Use {{variable_name}}.")
      return
    }
    const existingNames = new Set(variables.map((v) => v.name.trim()))
    const newVariables: PromptVariable[] = []
    for (const name of detected) {
      if (!existingNames.has(name)) {
        newVariables.push({
          id: `variable-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          name,
          value: "",
        })
      }
    ...
    if (newVariables.length > 0) {
      setVariables((current) => [...current, ...newVariables])
      setNotice(`Detected and added ${newVariables.length} prompt variable${newVariables.length > 1 ? "s" : ""}.`)
    } else {
      setNotice("All detected variables are already listed.")
    }
    setIsVariablePanelOpen(true)
  }
  ...
  async function submitPrompt(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessages((current) =>
        current.map((message) => (message.id === assistantId ? { ...message, status: "complete" } : message))
      )
  ...
  return (
    <main className="min-h-svh bg-background text-foreground">
      <header className="flex h-14 items-center justify-between border-b px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 border px-2 py-1 text-xs">
            <span
            ...
            />
            <span className="hidden font-mono text-[10px] sm:inline text-muted-foreground">
              {apiOnline === true ? "API Online" : apiOnline === false ? "API Offline" : "Connecting…"}
            </span>
          </div>
          <TooltipTrigger>
            <Button
              onPress={() => void refreshControlPlane()}
              variant="outline"
              size="icon-sm"
            ...
            <Tooltip>Refresh provider, history, and metrics data</Tooltip>
          </TooltipTrigger>
          <ThemeToggle />
          <Button onPress={openNewRun} size="sm">
            <Plus /> New run <Command className="ml-1 size-3" />
          </Button>
        </div>
      </header>
      ...
      <div className="grid min-h-[calc(100svh-3.5rem)] lg:grid-cols-[236px_minmax(0,1fr)_320px]">
        <aside className="hidden border-r p-3 lg:flex lg:flex-col">
          <SidebarContent
            ...
            onDeleteConversation={(id, e) => void deleteConversation(id, e)}
          />
        </aside>
        ...
        <section className="flex min-w-0 flex-col border-r">
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <p className="truncate text-sm font-medium">{currentViewTitle}</p>
                </div>
              ...
              {activeNav === "Playground" && (
                <>
                  <Button
                    ...
                    size="sm"
                    className="lg:hidden"
                    onPress={() => setIsInspectorOpen(true)}
                    aria-label="Toggle inspector settings"
                  >
                    <SlidersHorizontal className="size-3.5" /> Inspector
                  </Button>
                  <Button onPress={startNewRun} variant="outline" size="sm">
                    <RotateCcw /> Reset
                  </Button>
                </>
              )}
          ...
          {notice && (
            <div className="mx-4 mt-4 flex items-start gap-2 border bg-muted px-3 py-2 text-xs leading-5 sm:mx-6">
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/35 px-4 py-2 sm:px-6">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>{provider ? `${provider.name} / ${provider.model}` : "Connecting to API"}</span>
                </div>
                <span className="text-[10px] tracking-[0.1em] text-muted-foreground uppercase">
                  {summary.completed_runs}/{summary.total_runs} completed · {formatTokens(summary.total_tokens)} tokens
                </span>
              </div>
                    ...
                    <Card className="border">
                      <CardHeader className="border-b">
                        <div className="flex items-center justify-between gap-3">
                          <CardTitle>Start a prompt run</CardTitle>
                          <Badge variant="outline">Ready</Badge>
                        </div>
                        <CardDescription>
                          Choose a provider, adjust the request settings, then send a prompt. Live requests use
                          credentials configured on the server.
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="grid gap-3 pt-4 sm:grid-cols-3">
                        {["System instruction", "Model settings", "Run telemetry"].map((item, index) => (
                          <div key={item} className="border-l pl-3">
                            <p className="text-[10px] text-muted-foreground">0{index + 1}</p>
                            <p className="mt-1 text-xs font-medium">{item}</p>
                          </div>
                        ))}
                      </CardContent>
                    </Card>
                  ...
                  {messages.map((message) => (
                    <article key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : ""}`}>
                      {message.role === "assistant" && (
                        <div className="mt-0.5 grid size-7 shrink-0 place-items-center border">
                          <Bot className="size-3.5" />
                        </div>
                      )}
                      <div className={message.role === "user" ? "max-w-[82%]" : "min-w-0 max-w-2xl flex-1"}>
                        <div className="mb-1.5 flex items-center gap-2 text-[10px] tracking-[0.1em] text-muted-foreground uppercase">
                          <span>{message.role === "user" ? "You" : message.model ?? "Assistant"}</span>
                          {message.role === "assistant" && (
                            <Badge variant="outline">{message.status === "error" ? "Issue" : mode}</Badge>
                          )}
                        </div>
                        <div
                          className={`border px-3.5 py-3 text-xs leading-6 ${
                            message.role === "user" ? "bg-muted whitespace-pre-wrap" : "bg-card"
                          }`}
                        >
                          {message.content ? (
                            message.role === "assistant" ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-muted-foreground">
                              <Activity className="size-3 animate-spin" /> Generating response
                            </span>
                          )}
                        </div>
                        {message.role === "assistant" && message.content && message.status !== "streaming" && (
                          <div className="mt-1.5 flex items-center gap-1">
                            <TooltipTrigger>
                              ...
                              <Tooltip>{copiedId === message.id ? "Copied" : "Copy response"}</Tooltip>
                            </TooltipTrigger>
                            <TooltipTrigger>
                              <Button
                                onPress={() => setPrompt(message.content)}
                                variant="ghost"
                                size="icon-xs"
                                aria-label="Use response as prompt"
                              >
                                <RotateCcw />
                              </Button>
                              <Tooltip>Use as a new prompt</Tooltip>
                            </TooltipTrigger>
                          </div>
                        )}
                      </div>
                      {message.role === "user" && (
                        <div className="mt-0.5 grid size-7 shrink-0 place-items-center border bg-muted">
                ...
                <div className="mx-auto max-w-3xl border bg-card p-2">
                  {isVariablePanelOpen && (
                    <div className="mb-2 border bg-muted/35 p-3">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-xs font-medium">Prompt variables</p>
                          <p className="mt-0.5 text-[10px] text-muted-foreground">
                            Use <code>{"{{name}}"}</code> in the prompt or system instruction.
                          </p>
                        </div>
                        <div className="flex items-center gap-1">
                          <Button
                            type="button"
                            onPress={extractVariablesFromPrompt}
                            variant="outline"
                            size="xs"
                          >
                            <Wand2 className="size-3" /> Auto-detect
                          </Button>
                          <Button
                            type="button"
                            onPress={() => setIsVariablePanelOpen(false)}
                            variant="ghost"
                            size="icon-xs"
                      ...
                      <Button type="button" onPress={addVariable} variant="outline" size="sm" className="mt-3">
                        <Plus /> Add variable
                      </Button>
                    </div>
                  )}
                  ...
                  <div className="mt-2 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <TooltipTrigger>
                        <Button
                          type="button"
                          onPress={() => setIsVariablePanelOpen((open) => !open)}
                          variant={isVariablePanelOpen ? "secondary" : "ghost"}
                          size="icon-xs"
                          aria-label="Manage prompt variables"
                        >
                          <Variable />
                        </Button>
                        <Tooltip>Insert and resolve prompt variables</Tooltip>
                      </TooltipTrigger>
                    ...
                    {isStreaming ? (
                      <Button type="button" onPress={stopStream} variant="destructive" size="sm">
                        <Square className="size-3 fill-current" /> Stop
                      </Button>
                    ) : (
                      <Button
                        type="submit"
                        isDisabled={!prompt.trim() || !provider}
                        size="sm"
                      >
                        <SendHorizontal /> Run prompt
                      </Button>
                    )}
                  </div>
                </div>
              ...
              <div className="mx-auto max-w-4xl">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Open any persisted conversation or filter the local run archive.
                    </p>
                  </div>
                  <Button
                    onPress={() => void refreshControlPlane()}
                ...
                <div className="mt-6 grid gap-px border bg-border sm:grid-cols-3">
                  <div className="bg-card p-4">
                    <p className="text-[10px] tracking-[0.12em] text-muted-foreground uppercase">Total runs</p>
                    <p className="mt-2 font-mono text-xl">{summary.total_runs}</p>
                  </div>
                  <div className="bg-card p-4">
                    <p className="text-[10px] tracking-[0.12em] text-muted-foreground uppercase">Completed</p>
                    <p className="mt-2 font-mono text-xl">{summary.completed_runs}</p>
                  </div>
                  <div className="bg-card p-4">
                    <p className="text-[10px] tracking-[0.12em] text-muted-foreground uppercase">Average latency</p>
                    <p className="mt-2 font-mono text-xl">{formatLatency(summary.average_latency_ms)}</p>
                  </div>
                </div>
                <div className="mt-6">
                  <Input
                ...
                <div className="mt-4 space-y-2">
                  {visibleConversations.length ? (
                    visibleConversations.map((conversation) => (
                      <Card key={conversation.id} size="sm" className="border">
                        <CardContent className="flex flex-wrap items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              onPress={(e) => void deleteConversation(conversation.id, e as unknown as React.MouseEvent)}
                              variant="ghost"
                              size="icon-sm"
              ...
              <div className="mx-auto max-w-4xl">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Choose a route for Demo mode or view the environment key needed for Live mode.
                    </p>
                  </div>
                  <Button
                    onPress={() => void refreshControlPlane()}
                    ...
                    isDisabled={isRefreshing}
                  >
                    <RefreshCw className={isRefreshing ? "animate-spin" : ""} /> Refresh status
                  </Button>
                </div>
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  {providers.map((item) => (
                    <Card
                      key={item.id}
                      className={`border ${selectedProviderId === item.id ? "border-foreground" : ""}`}
                    >
                      <CardHeader className="border-b">
                        <div className="flex items-center justify-between gap-3">
                          <CardTitle>{item.name}</CardTitle>
                          <Badge variant={item.configured ? "default" : "outline"}>
                            {item.configured ? "Live ready" : "Demo ready"}
                          </Badge>
                        </div>
                        <CardDescription className="font-mono">{item.model}</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3 pt-4">
                        <div>
                          <p className="text-[10px] tracking-[0.12em] text-muted-foreground uppercase">Live route</p>
                          <p className="mt-1 break-all font-mono text-xs">{item.live_model}</p>
                        </div>
                        <div>
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {item.configured
                              ...
                              : `${item.required_env} is not configured.`}
                          </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <Button onPress={() => selectProvider(item.id)} size="sm">
                            Use in playground <ChevronRight />
                          </Button>
                          {item.required_env && (
                            <Button
                              onPress={() => void copyText(`${item.required_env}=`, `env-${item.id}`)}
                              variant="outline"
                              size="sm"
                            >
                              {copiedValue === `env-${item.id}` ? (
                                <>
                                  <Check /> Copied
                                </>
                              ) : (
                ...
                {!providers.length && (
                  <Card className="mt-6 border">
                    <CardContent className="py-8 text-center text-xs text-muted-foreground">
                      No provider routes are available. Refresh after starting the local API.
                    </CardContent>
                  </Card>
                )}
              </div>
        ...
        <aside className="hidden border-b lg:block lg:border-b-0">
      ...
      {isNavigationOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="relative flex h-full w-[min(20rem,88vw)] flex-col border-r bg-background p-3 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <PromptDeckLogo className="size-6" gradientId="promptdeck-logo-gradient-nav" />
                <p className="text-sm font-semibold">Workspace</p>
              </div>
              <Button onPress={() => setIsNavigationOpen(false)} variant="ghost" size="icon-sm" aria-label="Close navigation">
                <X />
              </Button>
            </div>
            <SidebarContent
      ...
      {isInspectorOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="relative ml-auto flex h-full w-[min(22rem,92vw)] flex-col border-l bg-background shadow-xl">
            <div className="flex h-12 items-center justify-between border-b px-4">
              <p className="text-xs font-semibold">Request Inspector</p>
              <Button onPress={() => setIsInspectorOpen(false)} variant="ghost" size="icon-sm" aria-label="Close inspector">
                <X />
              </Button>
            </div>
